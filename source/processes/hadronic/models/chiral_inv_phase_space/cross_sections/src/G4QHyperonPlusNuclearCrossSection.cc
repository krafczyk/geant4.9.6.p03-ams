...
//
//
// G4 Physics class: G4QHyperonPlusNuclearCrossSection for gamma+A cross sections
...
G4double G4QHyperonPlusNuclearCrossSection::CalculateCrossSection(G4bool, G4int F, G4int I,
                                        G4int, G4int targZ, G4int targN, G4double Momentum)
{
  ...
  {
#ifdef debug
    G4cout<<"G4QHyPNCS::CalcCS:bLEN nL="<<nL<<",TH="<<THmin<<",dP="<<dP<<G4endl;
#endif
    sigma=EquLinearFit(Momentum,nL,THmin,dP,lastLEN);
#ifdef debugn
    if(sigma<0.)
      G4cout<<"G4QHyPNuCS::CalcCS: E="<<Momentum<<",T="<<THmin<<",d="<<dP<<G4endl;
#endif
  }
...
#ifdef debug
  G4cout<<"G4QHyperonPlusNuclearCrossSection::CalcCS: CS="<<sigma<<G4endl;
#endif
  if(sigma<0.) return 0.;
  return sigma;
}
...
G4double G4QHyperonPlusNuclearCrossSection::ThresholdMomentum(G4int tZ, G4int tN)
{
  ...
  G4double dE=65.88;            // M_SigmaPlus-M_Lambda-M_PiPlus
...
G4double G4QHyperonPlusNuclearCrossSection::CrossSectionLin(G4int tZ, G4int tN, G4double P)
{
...
G4double G4QHyperonPlusNuclearCrossSection::CrossSectionLog(G4int tZ, G4int tN,G4double lP)
{
  G4double P=std::exp(lP);
  return CrossSectionFormula(tZ, tN, P, lP);
}
// Calculation formula for piMinus-nuclear inelastic cross-section (mb) log(P in GeV/c)
G4double G4QHyperonPlusNuclearCrossSection::CrossSectionFormula(G4int tZ, G4int tN,
                                                              G4double P, G4double lP)
{
  G4double sigma=0.;
  if(tZ==1 && !tN)                        // SigmaPlus-P interaction from G4QuasiElRatios
  {
    G4double ld=lP-3.5;
    G4double ld2=ld*ld;
    G4double p2=P*P;
  ...
  else if(tZ<97 && tN<152)                // General solution
  {
    ...
    if(tZ>1 || tN>1)
    {
      e=99.+146.*a2/(1.+a2/12.)/(1.+2.E-19*a8);
      r=0.15;
  ...
  {
    G4cerr<<"-Warning-G4QHyperPlNuclearCroSect::CSForm:*Bad A* Z="<<tZ<<", N="<<tN<<G4endl;
    sigma=0.;
  }
  if(sigma<0.) return 0.;
  return sigma;  

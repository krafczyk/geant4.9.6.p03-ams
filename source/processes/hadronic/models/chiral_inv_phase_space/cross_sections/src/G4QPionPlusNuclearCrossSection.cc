...
//
//
// G4 Physics class: G4QPionPlusNuclearCrossSection for gamma+A cross sections
...
// ****************************************************************************************
// Short description: CHIPS cross-sections for pi(plus)-nuclear interactions
// -------------------------------------------------------------------------------------
//
//#define debug
//#define pdebug
...
G4double* G4QPionPlusNuclearCrossSection::lastLEN=0; // Pointer to lastArray of LowEn CS
G4double* G4QPionPlusNuclearCrossSection::lastHEN=0; // Pointer to lastArray of HighEn CS
G4int     G4QPionPlusNuclearCrossSection::lastN=0;   // The last N of calculated nucleus
G4int     G4QPionPlusNuclearCrossSection::lastZ=0;   // The last Z of calculated nucleus
G4double  G4QPionPlusNuclearCrossSection::lastP=0.;  // Last used in cross section Momentum
G4double  G4QPionPlusNuclearCrossSection::lastTH=0.; // Last threshold momentum
G4double  G4QPionPlusNuclearCrossSection::lastCS=0.; // Last value of the Cross Section
G4int     G4QPionPlusNuclearCrossSection::lastI=0;   // The last position in the DAMDB
std::vector<G4double*>* G4QPionPlusNuclearCrossSection::LEN = new std::vector<G4double*>;
std::vector<G4double*>* G4QPionPlusNuclearCrossSection::HEN = new std::vector<G4double*>;

// Returns Pointer to the G4VQCrossSection class
G4VQCrossSection* G4QPionPlusNuclearCrossSection::GetPointer()
{
  static G4QPionPlusNuclearCrossSection theCrossSection; //**Static body of Cross Section**
...
G4double G4QPionPlusNuclearCrossSection::GetCrossSection(G4bool fCS, G4double pMom,
...
G4double G4QPionPlusNuclearCrossSection::CalculateCrossSection(G4bool, G4int F, G4int I,
                                        G4int, G4int targZ, G4int targN, G4double Momentum)
{
  ...
  {
#ifdef debug
    G4cout<<"G4QPipNCS::CalcCS:bLEN nL="<<nL<<",TH="<<THmin<<",dP="<<dP<<G4endl;
#endif
    sigma=EquLinearFit(Momentum,nL,THmin,dP,lastLEN);
#ifdef debugn
    if(sigma<0.)
      G4cout<<"G4QPipNuCS::CalCS: E="<<Momentum<<",T="<<THmin<<",dP="<<dP<<G4endl;
#endif
  }
...
#ifdef debug
  G4cout<<"G4QPionPlusNuclearCrossSection::CalcCS: CS="<<sigma<<G4endl;
#endif
  if(sigma<0.) return 0.;
  return sigma;
}
...
G4double G4QPionPlusNuclearCrossSection::ThresholdMomentum(G4int tZ, G4int tN)
{
  static const G4double third=1./3.;
  static const G4double pM = G4QPDGCode(211).GetMass(); // Projectile mass in MeV
...
G4double G4QPionPlusNuclearCrossSection::CrossSectionLin(G4int tZ, G4int tN, G4double P)
{
...
G4double G4QPionPlusNuclearCrossSection::CrossSectionLog(G4int tZ, G4int tN, G4double lP)
{
  G4double P=std::exp(lP);
  return CrossSectionFormula(tZ, tN, P, lP);
}
// Calculation formula for piMinus-nuclear inelastic cross-section (mb) log(P in GeV/c)
G4double G4QPionPlusNuclearCrossSection::CrossSectionFormula(G4int tZ, G4int tN,
...
#ifdef pdebug
    G4cout<<"G4QPiPlusNucCS::CSForm: A="<<a<<",P="<<P<<",CS="<<sigma<<",c="<<c<<",g="<<g_value
          <<",d="<<d<<",r="<<r<<",e="<<e<<",h="<<h<<G4endl;
#endif
  }
  else
  {
    G4cerr<<"-Warning-G4QPiPlusNuclearCroSect::CSForm:*Bad A* Z="<<tZ<<", N="<<tN<<G4endl;
    sigma=0.;
  }
  if(sigma<0.) return 0.;
  return sigma;  

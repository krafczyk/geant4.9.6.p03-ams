...
G4double G4QAntiBaryonPlusNuclearCrossSection::CalculateCrossSection(G4bool, G4int F, G4int I,
...
G4double G4QAntiBaryonPlusNuclearCrossSection::CrossSectionLin(G4int tZ, G4int tN,
                                                               G4double P)
{
...
G4double G4QAntiBaryonPlusNuclearCrossSection::CrossSectionLog(G4int tZ, G4int tN,
                                                               G4double lP)
{
...
// Calculation formula for piMinus-nuclear inelastic cross-section (mb) log(P in GeV/c)
G4double G4QAntiBaryonPlusNuclearCrossSection::CrossSectionFormula(G4int tZ, G4int tN,
                                                                   G4double P, G4double lP)
{
  ...
  else if(tZ==1 && tN==1)
  {
    ...
    G4double p4=p2*p2;
    G4double r=lP-3.7;
    sigma=(0.6*r*r+67.+90.*std::exp(-lP*.666))/(1.+4.E-7/p4/p4);
  }
  else if(tZ<97 && tN<152)                // General solution
  {
    ...
    G4double h=.016*(1.+1.5E-8*a3*a2s)/a4;
    sigma=(c+d*d+r/sp)/(1.+h/p4/p4);
#ifdef pdebug
    G4cout<<"G4QAntiBarPlNucCS::CSForm: A="<<a<<",P="<<P<<",CS="<<sigma<<",c="<<c<<",g="<<g
          <<",d="<<d<<",r="<<r<<",e="<<e<<",h="<<h<<G4endl;
#endif
  }
  else
  {
    G4cerr<<"-Warning-G4QAntiBarPlNuclCroSect::CSForm:*Bad A* Z="<<tZ<<", N="<<tN<<G4endl;
    sigma=0.;
  }
  if(sigma<0.) return 0.;
  return sigma;  
